    ...
    m_mainWindow(main)
{
    ...
    m_model = new TemplateListModel(m_mainWindow, this);
    ...
    if(fileList.count()>0)
    {
        ...
        {
            m_model->append(m_mainWindow->openFile(fileList.at(i).filePath()));
        }
    }
...
void TemplateDock::createFileListView(QFileInfoList &fileList, QObject *parent)
{
    ...
    for(int i=0; i<fileList.count(); i++)
    {
       if(fileList.at(i).fileName().contains(".mmp"))
       {
...
void TemplateDock::resizeEvent(QResizeEvent* event)
{
    for(TemplateListView *listView : *m_listviewList)
    {
        ...
        listView->setFixedHeight(row*hSize +5);
    }
