...
static bool copyData(QIODevice &inFile, QIODevice &outFile, int (*pCallBack)(int, void*), void *pUserData)
{
    while (!inFile.atEnd()) {
        char buf[4096];
        qint64 readLen = inFile.read(buf, 4096);

        if(pCallBack(int(readLen), pUserData) != 0)
        {
            return false;
        }
        if (readLen <= 0)
...
bool JlCompress::compressSubDir(QuaZip* zip, QString dir, QString origDir, bool recursive, QDir::Filters filters) {
	...
	if (dir != origDir) {
		...
		if (!dirZipFile.open(QIODevice::WriteOnly,
            QuaZipNewInfo(origDirectory.relativeFilePath(dir) + "/", dir), nullptr, 0, 0)) {
				return false;
		}
		dirZipFile.close();
	}
...
bool JlCompress::compressDir(QString fileCompressed, QString dir, bool recursive) {
    return compressDir(fileCompressed, dir, recursive, nullptr);
}
...
bool JlCompress::compressFile(QuaZip* pZip, QString strFileName, QString strFileDest, int (*pCallBack)(int, void*), void *pUserData)
{
    // zip: oggetto dove aggiungere il file
    // fileName: nome del file reale
    // fileDest: nome del file all'interno del file compresso
    ...
    if (!pZip)
    {
        qDebug() << "Error: zip is not exists.";
        return false;
    }

    if (pZip->getMode()!=QuaZip::mdCreate &&
        pZip->getMode()!=QuaZip::mdAppend &&
        pZip->getMode()!=QuaZip::mdAdd)
    {
        qDebug() << "Error: Zip mode is wrong!";
        return false;
    }

    // Apro il file originale
    QFile inFile;
    inFile.setFileName(strFileName);
    if(!inFile.open(QIODevice::ReadOnly))
    {
        qDebug() << "Error: Can't open inFile:" << strFileName;
        return false;
    }
    ...
    // fileDest:
    if(!outFile.open(QIODevice::WriteOnly, QuaZipNewInfo(strFileDest, inFile.fileName())))
    {
        inFile.close();

        qDebug() << "Error: Can't open outFile:" << pZip;
        return false;
    }

    // Copio i dati
    if (!copyData(inFile, outFile, pCallBack, pUserData) || outFile.getZipError()!=UNZ_OK)
    {
        outFile.close();
        inFile.close();

        qDebug() << "Error: Compress fail!";
        return false;
    }
    ...
    if (outFile.getZipError()!=UNZ_OK)
    {
        ...
        qDebug() << "Error: ZipError code is not UNZ_OK";
        return false;
    }
...
bool JlCompress::compressDir(QuaZip* pZip, QString strDir, QString strOrigDir, int (*pCallBack)(int, void*), void *pUserData)
{
    // zip: oggetto dove aggiungere il file
    // dir: cartella reale corrente
    // origDir: cartella reale originale
    ...
    if (!directory.exists())
    {
        qDebug() << "Error: Directory is not exists:" << strDir;
        return false;
    }

    QDir origDirectory(strOrigDir);

    if (strDir != strOrigDir)
    {
        QuaZipFile dirZipFile(pZip);
        if (!dirZipFile.open(QIODevice::WriteOnly, QuaZipNewInfo(strOrigDir + "/", strDir), nullptr, 0, 0))
        {
            qDebug() << "Error: Can't open outZip:" << pZip;
            return false;
        }
    ...
    {
        // Comprimo la sotto cartella
        QString strNewDir = strOrigDir + "/" + dir.fileName();
        if(!compressDir(pZip,dir.absoluteFilePath(),strNewDir, pCallBack, pUserData))
        {
            qDebug() << "Error: Compress directory fail!";
            return false;
        }
    }
    ...
    {
        // Se non e un file o e il file compresso che sto creando
        if(!file.isFile() || file.absoluteFilePath()==pZip->getZipName())
        {
            continue;
        }

        if (!compressFile(pZip,file.absoluteFilePath(), strOrigDir + "/" + file.fileName(), pCallBack, pUserData))
        {
            qDebug() << "Error: Compress file fail!" << file.absoluteFilePath();
            return false;
        }
    }
