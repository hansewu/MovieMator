...
static void transport( mlt_producer producer, mlt_consumer consumer , MELT_CALLBACK callback, void *callback_obj)
{
	mlt_properties properties = MLT_PRODUCER_PROPERTIES( producer );
			...
			if ( !silent && mlt_properties_get_int( properties, "stats_off" ) == 0 )
			{
				if ( progress )
				{
					...
					if ( current_position > last_position )
					{
                        ...
                        //回调通知进度
                        int ret = callback(0, 0, 100 * current_position / total_length, callback_obj);
                        if (ret != 0)
                            mlt_properties_set_int( properties, "done", 1);
...
int melt_main( int argc, char **argv, MELT_CALLBACK callback, void *callback_obj)
{
	int i;
    melt = NULL;
...
//#endif
                ...
                transport( melt, consumer , callback, callback_obj);
				
				// Stop the consumer
				mlt_consumer_stop( consumer );

                //回调通知执行成功完成
                callback(0, 1, 100, callback_obj);
			}	
		}
