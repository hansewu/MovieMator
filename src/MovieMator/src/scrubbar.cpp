...
// 使用的地方被注释了，消除警告
static const int selectionSize = 20; /// the height of the top bar
#ifndef CLAMP
...
void ScrubBar::setScale(int maximum)
{
    ...
    if (qFuzzyIsNull(m_scale)) m_scale = -1;
...
void ScrubBar::mouseMoveEvent(QMouseEvent * event)
{
    ...
    if (event->buttons() & Qt::LeftButton) {
        if (m_activeControl == CONTROL_IN && pos <= m_out)
            setInPoint(pos);
        else if (m_activeControl == CONTROL_OUT && pos >= m_in)
            setOutPoint(pos);
...
//        p.fillRect(0, 0, l_width, l_height, palette().background().color());
        p.fillRect(0, (l_height - 10) / 2, l_width, 10, palette().background().color());
        p.end();
        update();
        return;
    }
