...
public:
    ...
    int  getTrackType(int trackIndex);

    bool hasFilterApplied(Mlt::Producer *producer) const;

    int getStartPositionOfClip(int trackIndex, int clipIndex);

    void setClipSpeed(int trackIndex, int clipIndex, double speed);

    Mlt::Producer* producer(Mlt::Producer *producer, Mlt::Profile& profile, double speed);
signals:
    void created();
    void loaded();
    void closed();
...
public slots:
    ...
    void updateTransition(int trackIndex, int clipIndex);
