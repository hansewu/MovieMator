...
{
0x80,0x27,0x52,0x3a,0x45,0x18,0xc5,0x95,0x9c,0x33,0xae,0x55,0x59,0x18,
0x88,0xc9,0x37,0xa9,0x66,0x04,0x66,0xa0,0x4f,0x34,0x51,0x88,0xf7,0xe2
};

static unsigned char s_publicKey_Com[] =
{
0x17,0x1e,0x44,0xaa,0x75,0x5c,0x0e,0x6d,0xab,0x6b,0x29,0xe6,0x34,0xc9,0x59,
0x28,0xe7,0x2e,0x3b,0x64,0xb0,0x2f,0x24,0xe2,0x78,0x8b,0x3a,0xee
};
...
{
    0x17,0x1e,0x44,0xaa,0x75,0x5c,0x0e,0x6d,0xab,0x6b,0x29,0xe6,0x34,0xc9,0x59,
    0x28,0xe7,0x2e,0x3b,0x64,0xb0,0x2f,0x24,0xe2,0x78,0x8b,0x3a,0xed
};
...
//
int DoRsaChangeCommon(std::string strRegisterCode)
{
    int i, n = strRegisterCode.length();//, hexValue

#if defined(Q_OS_WIN)
    s_publicKey_Personal[11] = 0x6f;
    s_publicKey_Personal[4] = 0xdd;
    s_publicKey_Personal[14] = 0xca;
    s_publicKey_Personal[25] = 0x05;
#elif defined (Q_OS_MAC)
    ...
    for(i = 0; i < n; i ++)
    {
        if(i < 18 || i >= 27 )
        {
            mappedtext[mappedtextlen] = strRegisterCode.at(i);
            mappedtextlen ++;
        }
        if(i >= 18 && i < 24)
        {
            msg[msglen] = strRegisterCode.at(i);
            msglen ++;
        }
    }
...
//增加企业版
int DoRsaChangeCommercial(std::string strRegisterCode)
{
    int i, n = strRegisterCode.length();//, hexValue


#if defined(Q_OS_WIN)
    s_publicKey_Com[8] = 0xe3;
    s_publicKey_Com[2] = 0x55;
    s_publicKey_Com[24] = 0x3b;
    s_publicKey_Com[12] = 0x4c;
#elif defined (Q_OS_MAC)
    ...
    for(i = 0; i < n; i ++)
    {
        if(i < 18 || i >= 27 )
        {
            mappedtext[mappedtextlen] = strRegisterCode[i];
            mappedtextlen ++;
        }
        if(i >= 18 && i < 24)
        {
            msg[msglen] = strRegisterCode[i];
            msglen ++;
        }
    }
    Unmap32((unsigned char *)mappedtext, mappedtextlen, (unsigned char *)alphabet);
...
//增加家庭版
int DoRsaChangeFamily(std::string strRegisterCode)
{
    int i, n = strRegisterCode.length();//, hexValue

#if defined(Q_OS_WIN)
    s_publicKey_Fam[8] = 0xe3;
    s_publicKey_Fam[2] = 0x55;
    s_publicKey_Fam[24] = 0x3b;
    s_publicKey_Fam[12] = 0x4c;
#elif defined (Q_OS_MAC)
...
RegistrationType CEccRegister::checkRegisterInfo(std::string strRegisterCode)
{
    if(strRegisterCode.empty())
    {
        return Registration_None;
    }
    
    if(DoRsaChangeCommon(strRegisterCode))
    {
        return  Registration_Personal;
    }
    else if (DoRsaChangeCommercial(strRegisterCode))
    {
        return Registration_Commercial;
    }
    else if (DoRsaChangeFamily(strRegisterCode))
    {
...
RegistrationType CEccRegister::registration(std::string strRegisterCode)
{
    //企业版本跟家庭版本注册
//    std::replace(strRegisterCode.begin(), strRegisterCode.end(), '-', '\0');
    strRegisterCode.erase(remove(strRegisterCode.begin(), strRegisterCode.end(), '-'), strRegisterCode.end());
    RegistrationType type = CEccRegister::checkRegisterInfo(strRegisterCode);// [EccRegister CheckRegisterInfo:[strRegisterCode stringByReplacingOccurrencesOfString:@"-" withString:@""]];
    return type;
}
...
{
    return CEccRegister::registration(license);
}
