...
public:
    ...
    void run()
    {
        ...
        if (image.isNull()) {
            LOG_DEBUG()<<"playlistmodel makeThumbnail is called";
    ...
    , m_dropRow(-1)
{
    ...
    m_unbookmarkedFilesModel.setColumnCount(ColumnCount);
}
...
{
    Q_ASSERT(index.row() < m_playlist->count());
    Q_ASSERT(m_playlist->clip_info(index.row()));
